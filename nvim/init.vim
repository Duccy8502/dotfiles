...
" Plugins
	call plug#begin('/home/user/.config/nvim/vim-plug')
		...
		Plug 'georgewitteman/vim-fish'         " Add support for fishshell
		Plug 'cespare/vim-toml'                " Add support for TOML
	call plug#end()
...
" Includes
	" File settings (tabs, syntax, etc)
		" Assembly
		execute 'source /home/user/.config/nvim/patches/asm.vim'
...
" Plugin options
	" NerdTree
		...
		" and bring back NerdTree.
			...
			\ bufname('%') !~ 'NERD_tree_\d\+' && winnr('$') > 1 | let buf=bufnr() | buffer# | execute "normal! \<C-W>w" | execute 'buffer'.buf | endif
...
" Settings
	" Tabs
		set smarttab      " Insert amount of blanks (spaces) according to shiftwidth. A <BS> will delete shitfwidth of space
